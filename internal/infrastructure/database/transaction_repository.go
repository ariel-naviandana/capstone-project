...
func (r *transactionRepository) GetAccountBalance(ctx context.Context, accountNo string) (*domain.AccountBalance, error) {
    // 1. Update cache key menggunakan accountNo (string) dari temanmu
    cacheKey := fmt.Sprintf("account_balance:%s", accountNo)

    // 2. Pertahankan fitur Redis kamu, tapi ubah tipe datanya ke AccountBalance
    if cache.RedisClient != nil {
        if cached, hit := cache.GetCached[domain.AccountBalance](ctx, cacheKey); hit {
            return cached, nil
        }
    }

    // 3. Gunakan query dan eksekusi database dari branch temanmu
    ...
    // 4. Update populasi cache di bawah menggunakan accountNo
    if cache.RedisClient != nil {
        if setErr := cache.SetCache(ctx, cacheKey, result, 60*time.Second); setErr != nil {
            log.Warn().Err(setErr).Str("account_no", accountNo).Msg("GetAccountBalance: failed to populate cache")
        }
    }
