...
### Step 5 — Verifikasi API Health & PgBouncer Connection

```powershell
# Port-forward ke localhost:8080 (Windows Docker driver workaround)
kubectl port-forward svc/bankx-api 8080:8000
# (biarkan berjalan di terminal ini, buka terminal baru untuk test di bawah)

# Di terminal lain, test API health:
curl.exe "http://localhost:8080/health"
# Expected: {"status":"healthy","components":{"pgbouncer":"up","postgres_primary":"up",...}}

# Cek log API — verifikasi PgBouncer connection berhasil:
...
# Lihat status pool dari PgBouncer logs:
kubectl logs deploy/pgbouncer | Select-String "stats:"
# Expected: "stats: X xacts/s, Y queries/s, ..." (menunjukkan traffic mengalir)
```
